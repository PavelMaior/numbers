...
if __name__ == '__main__':
    while True:
        try:
            """
            при запуске контейнера постоянно была проблема, что скрипт начинал слать запросы до полной загрузки и 
            миграций БД и то падал то вис. Было принято решение запускать его после первого удачного запроса к бд 
            через бэк
            """
            logging.info(
